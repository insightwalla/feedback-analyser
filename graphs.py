...
def create_chart_totals_labels(data_frame, container_totals):
   ...
   labels = data_frame['Label: Dishoom'].tolist()
   ...
   labels = pd.DataFrame(labels.items(), columns=['Label: Dishoom', 'count'])

   fig = go.Figure()
   fig.add_trace(go.Bar(x=labels['Label: Dishoom'], y=labels['count'], opacity=0.8))
